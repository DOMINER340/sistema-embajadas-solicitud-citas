...
public class ISolicitarVisaEstudiante extends JPanel implements ActionListener{
	 ...
	 * Create the panel.
	 */
	public ISolicitarVisaEstudiante(InterfazGrafica interfaz)
	{
		...
		textMensaje.setText("Hacer solicitud visa Estudiante");
		...
		lblInstitucion.setFont(new Font("Times New Roman", Font.PLAIN, 22));
		lblInstitucion.setBounds(310, 20, 120, 30);
		panel.add(lblInstitucion);

		textInstitucion = new JTextField();
		textInstitucion.setBounds(450, 20, 120, 30);
		textInstitucion.setFont(new Font("Times New Roman", Font.PLAIN, 22));
		panel.add(textInstitucion);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			String Institucion = textInstitucion.getText();
			if(Institucion == null)
			{
				Institucion = "" ;
			}
			interfaz.crearSolicicitudEstudiante(Institucion);
		}
		if(accion.equals(InterfazGrafica.ADICIONAR_USUARIO))
		{
			...
			if( filaSeleccionada != (-1))
			{
				...
				//System.out.println("valor tomado num pasaporte: "+NumPassSeleccionado);

				int codigo = Integer.valueOf(textNoSolicitud.getText());
				//System.out.println("valor tomado codigo: "+codigo);
				interfaz.adicionarUsuario(NumPassSeleccionado,codigo);
			}
			else
			{
				//excepcion de qu no ah seleccionaedo ninguna fila
			}
